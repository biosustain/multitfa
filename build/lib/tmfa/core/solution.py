...
class Solution():
    ...
    def __init__(self, objective_value, status, fluxes, reduced_costs = None,
                shadow_prices = None, Gibbs_energies = None, metabolite_concentrations = None, solver = None):
...
def get_solution(model, reactions = None, metabolites = None, Gibbs_energy = None,
                 met_concentrations = None, raise_error = False):
    ...
    if Gibbs_energy is None:
        Gibbs_energy = [var for var in model.solver.variables if var.name.startswith('G_r_')]
    if met_concentrations is None:
        met_concentrations = [var for var in model.solver.variables if var.name.startswith('lnc_')]
    ...
    return Solution(model.solver.objective.value, model.solver.status,
                    ...
                    Series(index=met_conc_index, data=metabolite_concentrations, name="metabolite_concentrations"),
                    )