...
from numpy import array, dot, zeros, diag, diag_indices
...
class tmodel(Model):
    ...
    def cal_cholskey_matrix(self):
        ...
        Returns:
            [type] -- [description]
        """
        fake_cov_dg = zeros((len(self._cov_dg),len(self._cov_dg)))
        row,col = diag_indices(self._cov_dg.shape[0])
        fake_cov_dg[row,col]= diag(self._cov_dg)        

        chol_matrix = cholesky_decomposition(self._std_dg,fake_cov_dg)
